	...
	if (!fracht.empty()) {
		FOR(slist_tpl<ware_t>, & tmp, fracht) {
			...
			if(  !found  ) {
				...
				const int max_count = cnv->get_schedule()->get_entries().get_count();
				for(  int i=0;  i<max_count;  i++  ) {
					// try to unload on next stop
					halthandle_t halt = haltestelle_t::get_stoppable_halt( cnv->get_schedule()->get_entries()[ (i+offset)%max_count ].pos, cnv->get_owner() );
					if(  halt.is_bound()  ) {
...
bool rail_vehicle_t::check_longblock_signal(signal_t *sig, uint16 next_block, sint32 &restart_speed)
{
	...
	while(  schedule_index != cnv->get_schedule()->get_current_stop()  ) {
		...
		bool success = target_rt.calc_route( welt, cur_pos, cnv->get_schedule()->get_entries()[schedule_index].pos, this, speed_to_kmh(cnv->get_min_top_speed()), len );
...
bool rail_vehicle_t::can_couple(const route_t* route, uint16 start_index, uint16 &coupling_index, uint8 &coupling_steps, bool ignore_signals) {
	...
	do {
		if(  !cnv->is_waypoint(cnv->get_schedule()->get_entries()[idx].pos)  ) {
			stop_found = true;
			break;
		}
		idx = (idx+1)%cnv->get_schedule()->get_count();
	} while(  idx!=cnv->get_schedule()->get_current_stop()  );
	if(  !stop_found  ||  cnv->get_schedule()->get_entries()[idx].get_coupling_point()!=2  ) {
