...
koord3d private_car_t::find_destination(uint8 target_index) {
	...
	if(  direction90 != ribi_t::none  ) {
		...
		if(  rs_sign  &&  rs_sign->get_desc()->get_wtyp() == road_wt  &&  rs_sign->get_desc()->is_single_way()  &&  rs_sign->is_detailed_oneway()  ) {
		...
		if(  (ribi&ribi_t::nesw[r])!=0  ) {
			...
			if(  gr->get_neighbour(to, road_wt, ribi_t::nesw[r])  ) {
				...
				if(  w->has_sign()  ) {
					...
					if(  rs_to  &&  rs_to->get_desc()->get_wtyp() == road_wt  &&  rs_to->get_desc()->is_single_way()  &&  rs_to->is_detailed_oneway()  ) {
...
bool private_car_t::is_rerouting_needed() const{
	...
	// With ribi_maske=0 the normal ribi check above never catches this for 3-way junctions.
	{
		...
		if(  rs_n  &&  rs_n->get_desc()->get_wtyp() == road_wt  &&  rs_n->get_desc()->is_single_way()  &&  rs_n->is_detailed_oneway()  ) {
