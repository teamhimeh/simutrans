...
void calc_reachable_halts(vector_tpl<haltestelle_t::reachable_halt_t>& reachable_halts, convoihandle_t cnv) {
	...
	for(  uint8 i=1;  i<schedule_count;  i++  ) {
		...
		grund_t *gr = world()->lookup(schedule->get_entries()[wrap_i].pos);
		if(  gr  &&  gr->has_depot()  ) {
	...
	sint32 journey_time = -line_schedule->get_entries()[line_schedule_current_index].get_median_convoy_stopping_time(); 
	...
	for(  uint8 i=1;  i<line_schedule_count;  i++  ) {
		...
		const halthandle_t plan_halt = haltestelle_t::get_stoppable_halt(line_schedule->get_entries()[wrap_i].pos, owner);
		if(plan_halt == current_halt) {
			// we will come later here again ...
			break;
		}
		else if(  !plan_halt.is_bound()  ||  line_schedule->get_entries()[wrap_i].is_no_unload()  ) {
			// not a halt or set no_unload. no_unload -> we cannot unload the cargo there.
			continue;
		}
		const grund_t* gr = world()->lookup(line_schedule->get_entries()[wrap_i].pos);
		if(  gr  &&  gr->has_depot()  ) {
		...
		if(  line_schedule->get_entries()[wrap_i].is_unload_all()  ) {
		...
		else if( line_schedule->get_entries()[wrap_i].is_transfer_interval() ){
			if(interval == 1){
				break;
			}
			++interval;
...
void convoi_t::push_goods_waiting_time_if_needed() {
	...
	schedule_to_push->get_entry(schedule->get_current_stop()).push_waiting_time(average_waiting_time);
...
void convoi_t::push_convoy_stopping_time() {
	...
	if(  current_index_on_line_schedule>=0  ) {
		line_schedule->get_entry(current_index_on_line_schedule).push_convoy_stopping_time(stopping_time);
	}
}
...
void convoi_t::check_pending_updates()
{
	if(  line_update_pending.is_bound()  ) {
		...
		if (schedule->empty() || new_schedule->empty()) {
			...
			if(  current_stop<new_schedule->get_count() &&  current==new_schedule->get_entries()[current_stop].pos  ) {
				 ...
				 */
				const koord3d next = schedule->get_count()==0 ? koord3d::invalid : schedule->get_entries()[(current_stop+1)%schedule->get_count()].pos;
				const koord3d nextnext = schedule->get_count()==0 ? koord3d::invalid : schedule->get_entries()[(current_stop+2)%schedule->get_count()].pos;
				const koord3d nextnextnext = schedule->get_count()==0 ? koord3d::invalid : schedule->get_entries()[(current_stop+3)%schedule->get_count()].pos;
				int how_good_matching = 0;
				const uint8 new_count = new_schedule->get_count();

				for(  uint8 i=0;  i<new_count;  i++  ) {
					int quality =
						matches_halt(current,new_schedule->get_entries()[i].pos)*3 +
						matches_halt(next,new_schedule->get_entries()[(i+1)%new_count].pos)*4 +
						matches_halt(nextnext,new_schedule->get_entries()[(i+2)%new_count].pos)*2 +
						matches_halt(nextnextnext,new_schedule->get_entries()[(i+3)%new_count].pos);
					if(  quality>how_good_matching  ) {
						// better match than previous: but depending of distance, the next number will be different
						if(  matches_halt(current,new_schedule->get_entries()[i].pos)  ) {
							current_stop = i;
						}
						else if(  matches_halt(next,new_schedule->get_entries()[(i+1)%new_count].pos)  ) {
							current_stop = i+1;
						}
						else if(  matches_halt(nextnext,new_schedule->get_entries()[(i+2)%new_count].pos)  ) {
							current_stop = i+2;
						}
						else if(  matches_halt(nextnextnext,new_schedule->get_entries()[(i+3)%new_count].pos)  ) {
							current_stop = i+3;
						}
				...
				is_same = matches_halt(current,new_schedule->get_entries()[current_stop].pos);
			}
		}
...
bool convoi_t::can_start_coupling(convoi_t* parent) const {
	...
	*/
	...
	do {
		if(  !is_waypoint(schedule->get_entries()[t_idx].pos)  ) {
			stop_found = true;
			break;
		}
	...
	const schedule_entry_t t_c = schedule->get_entries()[t_idx];
	const schedule_entry_t t_n = schedule->get_entries()[(t_idx+1)%schedule->get_count()];
...
void convoi_t::register_journey_time() {
	...
	while(  c.is_bound()  ) {
		...
		if(  current_index_on_line_schedule>=0  ) {
			...
			line_schedule->get_entry(current_index_on_line_schedule).push_journey_time(journey_time);
		}
