...
bool dr_set_screen_scale(sint16 scale_percent)
{
	...
	if(  scale_percent == -1  ) {
		/* This branch is called before dr_os_init() in this repository. Keep
		 * the backend contract used by simsys_s2 and initialize video before
		 * asking SDL for displays. Newer Simutrans calls dr_os_init first, but
		 * changing that common startup order is unnecessary here. */
		if(  !SDL_WasInit( SDL_INIT_VIDEO )  &&  !SDL_InitSubSystem( SDL_INIT_VIDEO )  ) {
			dbg->warning( "dr_set_screen_scale(SDL3)", "cannot initialize video for automatic scaling: %s", SDL_GetError() );
			return false;
		}
...
static bool SDLCALL app_lifecycle_watch(void * /*userdata*/, SDL_Event *event)
{
	switch(  event->type  ) {
		...
		case SDL_EVENT_TERMINATING:
			// Quitting immediately: save the game and settings without visual feedback.
			intr_disable();
			...
			if(  env_t::reload_and_save_on_quit  &&  !env_t::networkmode  ) {
				const bool old_restore_UI = env_t::restore_UI;
...
bool dr_os_init(const int *parameter)
{
	...
	/* Keep the SDL version and selected video driver in every simu.log. This is
	 * the first information needed when a platform-specific backend issue is
	 * reported. */
...
int dr_os_open(int screen_width, int screen_height, sint16 fs)
{
	...
	window = SDL_CreateWindow( SIM_TITLE, screen_width, screen_height, flags );
	if(  window == NULL  ) {
	...
	DBG_MESSAGE("dr_os_open(SDL3)", "SDL realized screen size width=%d, height=%d (internal w=%d, h=%d)",
		screen_width, screen_height, tex_pitch, tex_h);
...
unsigned int get_system_color(unsigned int r, unsigned int g, unsigned int b)
{
	...
	const unsigned int ret = SDL_MapRGB( details, NULL, (Uint8)r, (Uint8)g, (Uint8)b );
...
void dr_flush()
{
	...
	const SDL_FRect src = { 0.0f, 0.0f, (float)min( display_get_width(), fb_pitch ), (float)min( display_get_height(), fb_height ) };
	...
	switch(  event.type  ) {
		...
		case SDL_EVENT_FINGER_MOTION: {
			...
			if(  fingers.count == 2  ) {
				 ...
				 * default scale. */
				const sint32 mx = (sint32)SCREEN_TO_TEX_X( fingers.cx * screen_width );
				const sint32 my = (sint32)SCREEN_TO_TEX_Y( fingers.cy * screen_height );
				...
				sys_event.mx      = (sint32)(event.tfinger.x * screen_width);
				sys_event.my      = (sint32)(event.tfinger.y * screen_height);
				...
				if(  FirstFingerId == event.tfinger.fingerID  ||  fingers.count == 0  ) {
					...
					if(  !previous_multifinger_touch  &&  !cancelled  ) {
						if(  dLastDist == 0.0  ) {
							...
							sys_event.mb      = MOUSE_LEFTBUTTON;
							sys_event.key_mod = ModifierKeys();
							last_mx = sys_event.mx = (sint32)(event.tfinger.x * screen_width);
							last_my = sys_event.my = (sint32)(event.tfinger.y * screen_height);
							...
							sys_event.code    = SIM_MOUSE_LEFTUP;
							sys_event.mb      = 0;
							sys_event.mx      = (sint32)((event.tfinger.x + event.tfinger.dx) * screen_width);
							sys_event.my      = (sint32)((event.tfinger.y + event.tfinger.dy) * screen_height);
							sys_event.key_mod = ModifierKeys();
						}
					}
					previous_multifinger_touch = 0;
		...
		case SDL_EVENT_TEXT_EDITING: {
			...
			for(  ; i < edit_start  &&  textinput[start];  ++i  ) {
				start = utf8_get_next_char( (const utf8 *)textinput, start );
			}
			size_t end = start;
			for(  ; i < edit_start + edit_length  &&  textinput[end];  ++i  ) {
				end = utf8_get_next_char( (const utf8 *)textinput, end );
			}
...
void dr_notify_input_pos(int x, int y)
{
	...
	const SDL_Rect rect = { (int)PIXEL_TO_WINDOW( TEX_TO_SCREEN_X(x) ), (int)PIXEL_TO_WINDOW( TEX_TO_SCREEN_Y(y + LINESPACE) ), 1, 1 };
	if(  window  ) {
