...
## Step 2c: Check for manual test mentions

Check the PR description (from `gh pr view`) and the review comments (from the `pulls/NUMBER/comments` endpoint) for any mention of manual testing (e.g., "手動テスト", "動作確認", "テスト済み").
If there is no mention of manual testing:
- You MUST execute the `simutrans-pr-manual-tests` skill to generate manual test proposals.
- Read `.agents/skills/simutrans-pr-manual-tests/SKILL.md` for instructions.
- Follow its steps to analyze changes, determine test cases, validate them via an independent agent, and handle ambiguous cases.
- **Important**: Instead of posting a separate PR comment (Step 6 of the manual-tests skill), you will include the resulting markdown checklist in the `body` of the review posted in Step 7 of this skill.
...
If you generated manual test cases in Step 2c, append the test case checklist to the `body` string below (make sure to properly escape newlines as `\n` in the JSON).
...
If there are no inline comments to post, but you have manual test cases to propose, you should still post the review (omitting the `comments` array if it is empty) to publish the test cases. If neither exist, report that no actionable issues were found and do not create an empty review.
