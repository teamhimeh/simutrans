...
void gui_journey_time_stat_t::update(linehandle_t line, vector_tpl<uint32*>& journey_times, vector_tpl<uint32*>& stopping_times, bool& empty_slot_exists) {
  ...
  for(uint8 idx=0; idx<schedule->get_entries().get_count(); idx++) {
    const schedule_entry_t& e = schedule->get_entries()[idx];
    const uint8 prev_idx = idx > 0 ? idx - 1 : schedule->get_entries().get_count() - 1;
...
void copy_csv_format(schedule_t* schedule, player_t* player, vector_tpl<uint32*>& journey_times) {
  ...
  for(uint8 i=0; i<schedule->get_entries().get_count(); i++) {
    halthandle_t const halt = haltestelle_t::get_stoppable_halt(schedule->get_entries()[i].pos, player);
    if(  halt.is_bound()  ) {
      clipboard.append(halt->get_name());
    } else {
...
void gui_journey_time_info_t::update() {
  ...
  for(uint8 i=journey_times.get_count(); i<schedule->get_entries().get_count(); i++) {
    journey_times.append(new uint32[NUM_ARRIVAL_TIME_STORED+1]);
  }
  for(uint8 i=stopping_times.get_count(); i<schedule->get_entries().get_count(); i++) {
  ...
  for(uint8 i=0; i<schedule->get_entries().get_count(); i++) {
    uint32 sum = 0;
    uint8 cnt = 0;
    const uint8 kc = (schedule->get_entries()[i].jt_at_index + NUM_ARRIVAL_TIME_STORED - 1) % NUM_ARRIVAL_TIME_STORED;
    for(uint8 k=0; k<NUM_ARRIVAL_TIME_STORED; k++) {
      const uint32* ca = schedule->get_entries()[i].journey_time;
  ...
  for(uint8 i=0; i<schedule->get_entries().get_count(); i++) {
    uint32 sum = 0;
    uint8 cnt = 0;
    const uint8 kc = (schedule->get_entries()[i].cs_at_index + NUM_STOPPING_TIME_STORED - 1) % NUM_STOPPING_TIME_STORED;
    for(uint8 k=0; k<NUM_STOPPING_TIME_STORED; k++) {
      const uint32* ca = schedule->get_entries()[i].convoy_stopping_time;
  ...
  bt_copy_names.enable(!schedule->get_entries().empty());
  bt_copy_stations.enable(!schedule->get_entries().empty());
  bt_copy_csv.enable(!schedule->get_entries().empty());
