...
void schedule_gui_stats_t::highlight_schedule(bool marking)
{
	...
	if(  marking  &&  schedule->get_current_stop() < schedule->get_count() ) {
		current_stop_mark->set_pos( schedule->get_entries()[schedule->get_current_stop()].pos );
...
{
	// compare schedules
	bool ok = (last_schedule != NULL)  &&  last_schedule->get_entries().get_count() == schedule->get_entries().get_count();
	for(uint i=0; ok  &&  i<last_schedule->get_entries().get_count(); i++) {
		ok = last_schedule->get_entry(i) == schedule->get_entry(i);
	}
	if (ok) {
		...
		if (schedule->empty()) {
			...
			for(uint i=0; i<schedule->get_entries().get_count(); i++) {
				entries.append( new_component<gui_schedule_entry_t>(player, schedule->get_entries()[i], i));
...
void schedule_gui_t::update_selection()
{
	...
	if(  !schedule->empty()  ) {
		...
		if(  haltestelle_t::get_stoppable_halt(schedule->get_entries()[current_stop].pos, player).is_bound()  ) {
			
			const uint8 c = schedule->get_entries()[current_stop].get_coupling_point();
			bt_find_parent.enable();
			bt_find_parent.pressed = c==2;
			bt_wait_for_child.enable();
			bt_wait_for_child.pressed = c==1;
			bt_no_load.enable();
			bt_no_load.pressed = schedule->get_entries()[current_stop].is_no_load();
			bt_no_unload.enable();
			bt_no_unload.pressed = schedule->get_entries()[current_stop].is_no_unload();
			bt_unload_all.enable();
			bt_unload_all.pressed = schedule->get_entries()[current_stop].is_unload_all();
			bt_load_before_departure.pressed = schedule->get_entries()[current_stop].is_load_before_departure();
			bt_transfer_interval.enable();
			bt_transfer_interval.pressed = schedule->get_entries()[current_stop].is_transfer_interval();
			bt_reverse_convoy.enable();
			bt_reverse_convoy.pressed = schedule->get_entries()[current_stop].is_reverse_convoy();
			bt_reverse_coupling.enable();
			bt_reverse_coupling.pressed = schedule->get_entries()[current_stop].is_reverse_convoi_coupling();

			
			// wait_for_time releated things
			const bool wft = schedule->get_entries()[current_stop].get_wait_for_time();
			bt_wait_for_time.enable();
			bt_wait_for_time.pressed = wft;
			if(  wft  ) {
				// enable departure time settings only
				//schedule->get_entries()[current_stop].spacing cannot be zero.
				const uint16 divisor = world()->get_settings().get_spacing_shift_divisor();
				const uint16 month_ratio_second = 86400/divisor;
				uint32 second = (86400/schedule->get_entries()[current_stop].spacing)/month_ratio_second*month_ratio_second;
				uint8 hour = second/3600;
				uint8 minute = (second - hour * 3600)/60;
				second = second % 60;
				sprintf(lb_spacing_str, "%d (%02d:%02d:%02d)", divisor/schedule->get_entries()[current_stop].spacing,hour,minute,second);

				uint32 second_shift = schedule->get_entries()[current_stop].spacing_shift * month_ratio_second;
				...
				numimp_load.enable();
				if(  schedule->get_entries()[current_stop].minimum_loading>0  ||  schedule->get_entries()[current_stop].get_coupling_point()!=0  ) {
					bt_wait_load.enable();
					uint16 wait = schedule->get_entries()[current_stop].waiting_time_shift;
					bt_wait_load.pressed = wait>0;
					if(  wait>0  ) {
						lb_wait.set_color( SYSCOL_TEXT );
						numimp_wait_load.enable();
						if(  schedule->get_entries()[current_stop].minimum_loading  ==  200  ){
							bt_load_before_departure.enable();
						}
					}
				}
			...
			numimp_load.set_value( schedule->get_entries()[current_stop].minimum_loading );
			numimp_wait_load.set_value( max(1, schedule->get_entries()[current_stop].waiting_time_shift) );
			numimp_spacing.set_value( schedule->get_entries()[current_stop].spacing );
			numimp_spacing_shift.set_value( schedule->get_entries()[current_stop].spacing_shift );
			numimp_delay_tolerance.set_value( schedule->get_entries()[current_stop].delay_tolerance );
		}
	}
}
	...
	if(comp == &bt_add) {
		...
		if(!schedule->empty()) {
			if(  bt_find_parent.pressed  ) {
				schedule->get_entry(schedule->get_current_stop()).reset_coupling();
			} else {
				schedule->get_entry(schedule->get_current_stop()).set_try_coupling();
			}
			bt_wait_for_child.pressed = false;
			schedule->get_entry(schedule->get_current_stop()).set_reverse_convoi_coupling(false);
			update_selection();
		}
	}
		...
		if(!schedule->empty()) {
			if(  bt_wait_for_child.pressed  ) {
				...
				schedule->get_entry(schedule->get_current_stop()).set_wait_for_coupling();
			}
		...
		if(!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_reverse_convoy(!bt_reverse_convoy.pressed);
			update_selection();
		}
	}
	else if(comp == &bt_reverse_coupling) {
		if(!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_reverse_convoi_coupling(!bt_reverse_coupling.pressed);
			if(  bt_wait_for_child.pressed  ) {
				schedule->get_entry(schedule->get_current_stop()).reset_coupling();
			} 
			if(  bt_find_parent.pressed  ) {
		...
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_minimum_loading((uint8)p.i);
			update_selection();
		}
	}
	else if(comp == &bt_wait_load) {
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_waiting_time_shift(!bt_wait_load.pressed);
			update_selection();
		}
	}
	else if(comp == &numimp_wait_load) {
		if(!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_waiting_time_shift((uint16)p.i);
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_no_load(!bt_no_load.pressed);
			update_selection();
		}
	}
	else if(comp == &bt_no_unload) {
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_no_unload(!bt_no_unload.pressed);
			update_selection();
		}
	}
	else if(comp == &bt_unload_all) {
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_unload_all(!bt_unload_all.pressed);
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_wait_for_time(!bt_wait_for_time.pressed);
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			if(  schedule->is_same_dep_time()  ) {
				...
				schedule->get_entry(schedule->get_current_stop()).set_spacing((uint16)p.i,schedule->get_current_entry().spacing_shift,schedule->get_current_entry().delay_tolerance);
			}
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			if(  schedule->is_same_dep_time()  ) {
				...
				schedule->get_entry(schedule->get_current_stop()).set_spacing(schedule->get_current_entry().spacing,(uint16)p.i,schedule->get_current_entry().delay_tolerance);
			}
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			if(  schedule->is_same_dep_time()  ) {
				...
				schedule->get_entry(schedule->get_current_stop()).set_spacing(schedule->get_current_entry().spacing,schedule->get_current_entry().spacing_shift,(uint16)p.i);
			}
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_load_before_departure(bt_load_before_departure.pressed);
			update_selection();
		}
	}
		...
		if (!schedule->empty()) {
			schedule->get_entry(schedule->get_current_stop()).set_transfer_interval(!bt_transfer_interval.pressed);
			update_selection();
		}
	}
