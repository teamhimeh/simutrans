		...
		if(sch) {
			...
			if(  left_swap  ) {
				...
				if(temp_dir&ribi_t::east) {
					set_back_image_id(3+state*4+offset);
					xoff += XOFF;
					yoff += -YOFF;
				}
				...
				if(temp_dir&ribi_t::north) {
					if(temp_dir&ribi_t::east) {
						set_front_image_id(0+state*4+offset);
						after_xoffset += -XOFF;
						after_yoffset += -YOFF;
					}
					else {
						set_back_image_id(0+state*4+offset);
						xoff += -XOFF;
						yoff += -YOFF;
					}
				}
				...
				if(temp_dir&ribi_t::west) {
					set_front_image_id(2+state*4+offset);
					after_xoffset += -XOFF;
					after_yoffset += YOFF;
				}
				...
				if(temp_dir&ribi_t::south) {
					if(temp_dir&ribi_t::west) {
						set_back_image_id(1+state*4+offset);
						xoff += XOFF;
						yoff += YOFF;
					}
					else {
						set_front_image_id(1+state*4+offset);
						after_xoffset += XOFF;
						after_yoffset += YOFF;
					}
				}
				...
				if(temp_dir&ribi_t::east) {
					set_front_image_id(3+state*4+offset);
				}
				...
				if(temp_dir&ribi_t::west) {
					set_back_image_id(2+state*4+offset);
				}
