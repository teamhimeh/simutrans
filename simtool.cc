		...
		for(  uint i=0;  i<2;  i++  ) {
				...
				if(!cnv->get_line().is_bound()  &&  cnv->get_owner()==player) {
					...
					if(schedule  &&  schedule->is_stop_allowed(bd)) {
						...
						for(uint8 i=0; i<schedule->get_count();i++) {
							if ((catch_all_halt && haltestelle_t::get_stoppable_halt( schedule->get_entry(i).pos, cnv->get_owner()) == last_halt) ||
									old_platform.is_contained(schedule->get_entry(i).pos)) {
								schedule->get_entry(i).pos   = pos;
								updated = true;
							}
						}
						if(updated) {
			...
			FOR(vector_tpl<linehandle_t>, const line, lines) {
				...
				if(schedule->is_stop_allowed(bd)) {
					...
					for(uint8 i=0; i<schedule->get_count(); i++) {
						// ok!
						if ((catch_all_halt && haltestelle_t::get_stoppable_halt( schedule->get_entry(i).pos, line->get_owner()) == last_halt) ||
								old_platform.is_contained(schedule->get_entry(i).pos)) {
							schedule->get_entry(i).pos   = pos;
							updated = true;
						}
