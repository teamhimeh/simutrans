...
uint32 estimated_waiting_ticks(const schedule_t* schedule, uint8 index) {
	const schedule_entry_t schedule_entry = schedule->get_entries()[index];
...
// DBG_MESSAGE("haltestelle_t::rebuild_destinations()", "Adding new table entries");
	...
	while(  lines  ||  current_index < registered_convoys.get_count()  ) {
		...
		uint8 start_index = 0;
		while(  start_index < schedule->get_count()  &&  get_stoppable_halt( schedule->get_entries()[start_index].pos, owner ) != self  ) {
			++start_index;
		}
		...
		const schedule_entry_t start_entry = schedule->get_entries()[start_index-1];
		...
		for(  uint8 j=0;  j<schedule->get_count();  ++j  ) {
			...
			const schedule_entry_t current_entry = schedule->get_entries()[current_entry_index];
...
bool unregistered_journey_time_exists(const schedule_t* schedule, player_t* player) {
	for(uint8 i=1; i<schedule->get_count()+1; i++) {
		const schedule_entry_t& this_entry = schedule->get_entries()[i%schedule->get_count()];
		const schedule_entry_t& prev_entry = schedule->get_entries()[i-1];
		if(  
