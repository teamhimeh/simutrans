...
bool ai_passenger_t::create_water_transport_vehikel(const stadt_t* start_stadt, const koord target_pos)
{
	...
	// now we must check, if these two seas are connected
	{
		...
		route_t verbindung;
		bool connected = verbindung.calc_route( welt, koord3d( start_harbour - start_dx, welt->get_water_hgt( start_harbour - start_dx ) ), koord3d( end_harbour - end_dx, welt->get_water_hgt( end_harbour - end_dx ) ), test_driver, 0, 0, { /* TODO: INT_CHECK */ });
		delete test_driver;
		if(!connected) {
			return false;
		}
