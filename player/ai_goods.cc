			...
			if(ship_vehicle) {
				...
				if(  start_halt.is_bound()  &&  (start_halt->get_station_type()&haltestelle_t::dock)!=0  ) {
					...
					if(!lines.empty()) {
						...
						if(schedule->get_count()>1  &&  haltestelle_t::get_halt(schedule->get_entries()[0].pos,this)==start_halt) {
			...
			for (size_t i = welt->convoys().get_count(); i-- != 0;) {
				...
				if(delete_this) {
					...
					if(schedule  &&  schedule->get_count()>1) {
						start_pos = schedule->get_entries()[0].pos;
						end_pos = schedule->get_entries()[1].pos;
					}
					...
					if(  !line.is_bound()  ||  line->count_convoys()==0  ) {
						...
						if(start_halt.is_bound()  &&  (start_halt->get_station_type()&haltestelle_t::dock)!=0) {
							...
							FOR(vector_tpl<linehandle_t>, const line, lines) {
								schedule_t *schedule=line->get_schedule();
								if(schedule->get_count()>1  &&  haltestelle_t::get_halt(schedule->get_entries()[0].pos,this)==start_halt) {
									water_stop = koord( (start_pos.x+schedule->get_entries()[0].pos.x)/2, (start_pos.y+schedule->get_entries()[0].pos.y)/2 );
									while(line->count_convoys()>0) {
										convoihandle_t cnv = line->get_convoy(0);
										cnv->self_destruct();
										if(cnv.is_bound()) {
